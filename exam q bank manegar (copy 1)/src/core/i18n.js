...
export const i18n = {
    ...
    t(key, params = {}) {
        ...
        let translation = this.locales[lang]?.[key] || this.locales['ar']?.[key] || key;
    ...
    updateUI() {
        document.querySelectorAll('[data-i18n]').forEach(el => {
            ...
            if (el.tagName === 'INPUT' || el.tagName === 'TEXTAREA') {
                ...
                // If it has children, we might want to preserve icons. 
                // A better way is to use data-i18n on the specific text node or use a naming convention.
                // For now, if there's no HTML inside, just replace text.
                // If there's an icon span, we should be careful.
                
                const icon = el.querySelector('span, i');
                if (icon && el.childNodes.length > 1) {
                    // Try to preserve the icon if it's the first child
