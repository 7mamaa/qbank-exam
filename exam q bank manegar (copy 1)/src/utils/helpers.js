...
export const Helpers = {
    ...
    async fetchUrlWithProxy(url) {
        ...
        for (let strategy of strategies) {
            ...
            } catch (e) { console.warn("Proxy strategy failed."); }
        }
    ...
    parseSmartText(text) {
        ...
        if (text.startsWith('[') || text.startsWith('{')) {
            try {
                ...
                // If it looks like JSON but fails, throw to allow syntax error debugging
                throw e;
            }
        }
    ...
    generatePrintHTML(title, qList, mode) {
        ...
        qList.forEach((q, idx) => {
            ...
            if (q.type === 'mcq') {
                ...
                if (mode === 'study') html += `<p>${i18n.t('print_keywords', {words: this.sanitize((q.keywords || []).join(', '))})}</p>`;
