...
export const QuizModule = {
    ...
    submitQuiz() {
        ...
        let score = 0;
        state.quizState.pool.forEach(q => {
            const userAns = state.quizState.answers[q.id];
            if (q.type === 'mcq' || q.type === 'boolean') {
                if (userAns === q.answer) score++;
            } else if (q.type === 'written') {
                const matches = (q.keywords || []).filter(k => userAns?.toLowerCase().includes(k.toLowerCase()));
                if (matches.length >= (q.keywords?.length || 0) / 2 && (q.keywords?.length || 0) > 0) score++;
            }
        });
