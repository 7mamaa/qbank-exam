...
export const UIComponents = {
    ...
    renderDuplicateManager(groups, totalScanned = 0) {
        ...
        if (summaryEl) {
            ...
            if (totalPairs === 0) {
                summaryEl.innerHTML = `<span class="dup-stat">✅ لا توجد تكرارات | تم فحص <strong>${totalScanned}</strong> سؤال</span>`;
            } else {
                summaryEl.innerHTML = `<span class="dup-stat">المجموعات: <strong>${groups.length}</strong></span><span class="dup-stat">الأزواج: <strong>${totalPairs}</strong></span>`;
            }
        }
        ...
        if (totalPairs === 0) {
            container.innerHTML = `<div class="dup-empty-state">🎉 البنك نظيف تماماً!</div>`;
            return;
        }
            ...
            group.duplicates.forEach(dup => {
                ...
                <div class="dup-card" data-org-id="${org.id}" data-dup-id="${dq.id}">
                    <div class="dup-card-header">
                        <span class="dup-pair-label">مجموعة تكرار #${pairIndex}</span>
                        <span class="dup-sim-badge">${sim}% تشابه</span>
                    </div>
                    ...
                    <div class="dup-compare-stack">
                        ...
                        <div class="dup-side-box original">
                            ...
                            <div class="dup-q-meta">
                                <span>📁 ${Helpers.sanitize(org.category || 'عام')}</span>
                        ...
                        <div class="dup-side-box duplicate">
                            ...
                            <div class="dup-q-meta">
                                <span>📁 ${Helpers.sanitize(dq.category || 'عام')}</span>
                    ...
                    <div class="dup-card-footer">
                        <div class="dup-card-actions">
                            <button class="btn btn-sm dup-action-merge" data-org="${org.id}" data-dup="${dq.id}" style="background:var(--primary); color:#000; border:none;">دمج وحذف المكرر</button>
                            <button class="btn btn-sm dup-action-edit" data-id="${dq.id}" style="background:var(--accent); color:white; border:none;">تعديل المكرر</button>
                            <button class="btn btn-sm dup-action-dismiss" style="background:transparent; border:1px solid var(--border); color:var(--text-secondary);">تخطي</button>
                        </div>
                    </div>
                </div>`);
            });
