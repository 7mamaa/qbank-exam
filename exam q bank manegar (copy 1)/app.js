...
import { NotebookModule } from './src/modules/notebooks.js';
import { QuestionModule } from './src/modules/questions.js';
import { QuizModule } from './src/modules/quiz.js';
import { ExportModule } from './src/modules/export.js';
import { AIModule } from './src/modules/ai.js';
import { UIComponents } from './src/ui/components.js';
...
const app = {
    ...
    async syncData(broadcast = true) {
        try {
            ...
            while (loadedQuestions.length < totalQuestions) {
                const batch = await db.getBatch('questions', 0, batchSize, lastKey);
    ...
    saveInstitution() {
        ...
        this.showToast('✅ تم حفظ هوية المؤسسة', 'success');
    },

    clearInstitution() {
        if (!confirm('هل أنت متأكد من مسح هوية المؤسسة (الاسم والشعار)؟')) return;
        ...
        this.showToast('🗑️ تم إزالة هوية المؤسسة بنجاح', 'info');
    },
    ...
    applyCustomTheme() {
        ...
        this.showToast('🎨 تم تطبيق الثيم المخصص', 'success');
    },
    ...
    initCustomTheme() {
        ...
        } catch(e) { /* ignore corrupt saved theme */ }
    },
    ...
    async validateAndImportJSON() {
        ...
        try {
            ...
            this.showToast('✅ تم استيراد الملف المُصلح بنجاح', 'success');
        } finally {
            this.hideLoading();
        }
    },
        ...
        document.getElementById('import-file')?.addEventListener('change', (e) => {
            ...
            reader.onload = async (event) => {
                ...
                if (file.name.endsWith('.json')) {
                    // Try to parse first — open JSON Fixer on error
                    try {
                        JSON.parse(raw);
                        await ExportModule.importData(raw, () => this.syncData());
                    } catch(parseErr) {
                        this.openJsonFixer(raw, parseErr.message);
                    }
                } else {
                    alert(i18n.t('err_only_json'));
                }
                e.target.value = '';
            };
    ...
    async exportData(fmt) { await ExportModule.exportData(fmt, (msg) => this.showToast(msg)); },
    ...
    async copyQuestion(id, format = 'json') {
        ...
        try {
            ...
            this.showToast('فشل النسخ — جرب مرة أخرى', 'error');
        }
    },
    ...
    async performAction(actionType, data, isUndoRedo = false) {
        if (!isUndoRedo) {
            ...
            this.history.undo.push({ type: actionType, state: snapshot, fullData: data });
    ...
    async undo() {
        ...
        await db.clearAll();
        for (let nb of lastAction.state.notebooks) await db.put('notebooks', nb);
        for (let q of lastAction.state.questions) await db.put('questions', q);
    ...
    async redo() {
        ...
        await db.clearAll();
        for (let nb of lastRedo.state.notebooks) await db.put('notebooks', nb);
        for (let q of lastRedo.state.questions) await db.put('questions', q);
    ...
    // === AI Module Bridge ===
    openAiPromptModal() { 
        this.openModal('ai-prompt-modal'); 
        this.regeneratePrompt();
    },

    regeneratePrompt() {
        AIModule.regeneratePrompt();
    },

    async copyAiPrompt() {
        const success = await AIModule.copyAiPrompt();
        if (success) this.showToast(i18n.t('msg_prompt_copied'), 'success');
        else this.showToast(i18n.t('msg_copy_fail'), 'error');
    },
    ...
    openTagNormalizer() {
        ...
        container.innerHTML = suggestions.map((s, idx) => `
            <div class="stat-card" style="padding:10px; display:flex; align-items:center; justify-content:space-between; border: 1px solid var(--border);">
                ...
                <input type="checkbox" class="tag-merge-check" data-from="${s.from}" data-to="${s.to}" checked style="width:20px; height:20px;">
            </div>
        `).join('');
    },
    ...
    openCategoryNormalizer() {
        ...
        container.innerHTML = suggestions.map((s, idx) => `
            <div class="stat-card" style="padding:10px; display:flex; align-items:center; justify-content:space-between; border: 1px solid var(--border);">
                ...
                <input type="checkbox" class="cat-merge-check" data-from="${s.from}" data-to="${s.to}" checked style="width:20px; height:20px;">
            </div>
        `).join('');
    },
    ...
    openDuplicateManager() {
        this.openModal('duplicate-manager-modal');
        const groups = QuestionModule.findDuplicates(0.8);
        UIComponents.renderDuplicateManager(groups);
    },

    async executeMerge(originalId, duplicateId) {
        if (!confirm(i18n.t('dup_merge_btn') + '?')) return;
        
        try {
            this.playSound('click');
            const success = await QuestionModule.mergeQuestions(originalId, duplicateId);
            if (success) {
                UIComponents.showToast(i18n.t('dup_merge_success'), 'success');
                // Re-scan and re-render remaining duplicates
                const groups = QuestionModule.findDuplicates(0.8);
                UIComponents.renderDuplicateManager(groups);
                await this.syncData(); 
            }
        } catch (err) {
            this.handleError(err, 'Merge Questions');
        }
    },
    ...
    async loadChangelog() {
        ...
        try {
            ...
            containers.forEach(container => {
                const isEn = container.id === 'changelog-content-en';
    ...
    bridgeToExam(type = 'default') {
        ...
        // Priority 1: Smart Selection criteria (Direct from Hub)
        if (type === 'smart') {
            pool = QueryEngine.getQueryPool(this.state.selectionCriteria);
            console.log(`Bridge: Using ${pool.length} questions from Smart Selection criteria.`);
        }
        // Priority 2: Manual selection (Checkboxes)
        else if (this.state.selectedIds.size > 0) {
            pool = this.state.questions.filter(q => this.state.selectedIds.has(q.id));
            console.log(`Bridge: Using ${pool.length} manually selected questions.`);
        } 
        // Priority 3: Random Quiz generation (from Quiz Setup)
        else if (type === 'quiz') {
        ...
        // Priority 4: Fallback to Export Hub filters or all
        else {
            pool = this.getQueryPool();
        }

        if (pool.length === 0) {
            UIComponents.showToast('⚠️ لا توجد أسئلة مختارة لنقلها.', 'warning');
            return;
        }
        ...
        // Prepare data in the format expected by the exam site
        const bridgeData = pool.map(q => {
            let options = q.options || [];
            let ans = q.answer;
            let isEssay = q.type === 'written';

            // Transform Boolean to MCQ for the exam site if it doesn't support Boolean natively
            if (q.type === 'boolean') {
                options = ['صح', 'خطأ'];
                ans = q.answer ? 'صح' : 'خطأ';
                isEssay = false;
            ...
            return {
                id: q.id,
                type: q.type,
                question: q.question, // Maps to item.question in examwebsite
                answer: ans,          // Maps to item.answer
                options: options,
                image: q.image || '',
                explain: q.explain || '',
        ...
        try {
            console.log(`🚀 Bridge: Preparing to send ${bridgeData.length} questions to exam site.`);
            localStorage.setItem(storageKey, JSON.stringify(bridgeData));
            const url = `examwebsite/index.html?session_id=${sessionId}`;
            window.open(url, '_blank');
            UIComponents.showToast(`🚀 جاري نقل ${bridgeData.length} سؤال إلى محاكي الاختبار...`, 'success');
        } catch (e) {
            this.handleError(e, "فشل عمل الجسر");
        }
    }
};
...
window.app = app;
window.i18n = i18n; // Expose i18n globally to avoid ReferenceErrors in modules
window.Helpers = Helpers; // Expose Helpers globally to avoid ReferenceErrors in modules during export
