...
export const ExportModule = {
    ...
    exportData: async function(fmt, showToastCallback) {
        const pool = QueryEngine.getQueryPool();
        if (pool.length === 0) {
        ...
        if (fmt === 'json') {
            ...
            pool.forEach(q => {
                ...
                else if (q.type === 'written') back = `${i18n.t('export_answer')}: ${q.answer || (q.keywords || []).join(' - ')}`;
    ...
    async importData(jsonText, syncCallback) {
        try {
            ...
            for (const item of data) {
                ...
                const sourceName = item.notebookName || (typeof item.notebookId === 'string' && isNaN(item.notebookId) ? item.notebookId : null);
